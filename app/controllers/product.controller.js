...
Product.create(product, (err, data) => {
    if (err)
        res.status(500).send({
        message: err.message || "apparition d'erreurs lors de la création d'un produit"
        });
    else res.send(data);
    });
};
...
exports.findOne = (req, res) => {
    ...
    res.status(500).send({
    message: "Erreur de récupération du produit avec l'identifiant " +
    req.params.id
    });
    }
    } else res.send(data);
...
exports.delete = (req, res) => {
    ...
    res.status(500).send({
    message: "Impossible de supprimé le produit avec l'id " + req.params.id
    });
    }
    ...
    if (err)
    res.status(500).send({
    message: err.message || "Apparition d'erreurs lors de la suppression de tous les produits."
    });
